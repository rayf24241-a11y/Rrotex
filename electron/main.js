...
async function startPluginServerInternal(token, projectName, proPass, projectMode) {
  ...
  return new Promise(resolve => {
    ...
    const tryNextPort = () => {
      ...
      // Keep connections alive long past the plugin's poll interval. A short
      // keep-alive (2s) made the server close idle sockets that Roblox's
      // HttpService then tried to reuse → intermittent "ConnectFail" / flapping.
