...
# The 3D stage can only reconstruct detail that was legible in the 2D
# reference image, so the reference generator is the real ceiling on face,
# hand and foot quality -- and SDXL-Turbo is distilled for 1-4 steps at
# guidance 0.0, which is exactly the regime where it garbles small
# high-frequency detail (see the fused-hands and blurred-face notes on
# TEXT_TO_IMAGE_STYLE_SUFFIX below). Full SDXL at real step counts and real
# guidance is a few seconds slower per image and much steadier on anatomy;
# the extra seconds fit the ~30s per-model budget. Turbo stays as a fallback
# so a missing/failed download degrades speed-quality instead of crashing.
TXT2IMG_CANDIDATES = [
    ("stabilityai/stable-diffusion-xl-base-1.0", 24, 6.5),
    ("stabilityai/sdxl-turbo", 6, 0.0),
]


def _load_txt2img():
    last_err = None
    for model_id, steps, guidance in TXT2IMG_CANDIDATES:
        try:
...
# seconds), so this is bounded by the same ~30s budget the rest of the
# pipeline is tuned to.
IMAGE_ATTEMPTS = 3
...
REVIEWER_QUESTION = (
    ...
    "the face is blurry, distorted, or has malformed features; the hands are "
    "fused together, hidden, or have the wrong number of fingers; the feet are "
...
def _review_image(image: Image.Image) -> Optional[str]:
    ...
    on the highest-stakes (safety) check. Returns the reviewer's complaint,
    or None when the image passes. Fails open on an unexpected error
    ...
    try:
        ...
        traceback.print_exc()
        return None
    return answer.strip() if answer.strip().upper().startswith("FAIL") else None


def _ai_review_image(image: Image.Image):
    complaint = _review_image(image)
    if complaint:
        raise ValueError(f"Image flagged by content reviewer: {complaint}")
...
TEXT_TO_IMAGE_STYLE_SUFFIX = (
    ...
    "detailed hands with five separate fingers, detailed feet, both feet fully visible"
)

# Only meaningful at guidance > 0, i.e. on the full-SDXL path -- Turbo at
# guidance 0.0 ignores it. States the same failure modes the positive suffix
# targets (garbled faces, fused or extra fingers, missing feet, duplicated
# limbs) as things to steer away from, which is the lever the positive
# prompt can't pull.
TEXT_TO_IMAGE_NEGATIVE_PROMPT = (
    "deformed face, distorted face, blurry face, disfigured, "
    "deformed hands, fused fingers, extra fingers, missing fingers, mangled hands, "
    "deformed feet, missing feet, cropped feet, cropped legs, cut off, out of frame, "
    "extra limbs, missing limbs, duplicated limbs, extra legs, extra arms, extra tail, "
    "multiple subjects, chair, furniture, props, floor, ground, shadow, "
    "low quality, low detail, blurry"
)
...
def _run_job(job_id: str):
    ...
    if req.image_b64:
        ...
        # Covers both paths -- an uploaded image never went through the
        # prompt keyword filter in /generate at all, and a text prompt that
        # dodges that filter still has to produce an actual image.
        ...
        # The reviewer already knows how to spot the exact composition
        # failures that ruin a model (body cut off, a second subject, props)
        # -- it was just being used to fail the job. A rejected reference
        # image costs a few seconds to replace and every retry is a fresh
        # sample, so retrying turns most of those failures into a good model
        # instead of an error. Safety rejections are NOT retried: those come
        # from _check_image_safety/the keyword filter and mean the request
        ...
        for attempt in range(IMAGE_ATTEMPTS):
            ...
            candidate = txt2img(
                ...
                guidance_scale=TXT2IMG_GUIDANCE,
            ).images[0]
            image_seconds += time.time() - t_img

            _check_image_safety(candidate)

            t_review = time.time()
            complaint = _review_image(candidate)
            review_seconds += time.time() - t_review
            image = candidate
            if complaint is None:
                break
            print(f"Reference image attempt {attempt + 1} rejected: {complaint}", flush=True)
    else:
